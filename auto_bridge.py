...
def get_api_quote(w3, from_toton, to_toton, amount_in_wei, user_address):
    log_message(f"Requesting quote from API for {from_toton} -> {to_toton}...")
    url = "https://api.relay.link/quote"
    headers = {"accept": "application/json", "content-type": "application/json"}
    payload = { "user": user_address, "originChainId": 57073, "destinationChainId": 57073, "originCurrency": from_toton, "destinationCurrency": to_toton, "recipient": user_address, "tradeType": "EXACT_INPUT", "amount": str(amount_in_wei), "referrer": "inkonchain.com", "useExternalLiquidity": False }
    try:
...
def execute_transaction(w3, account, tx_details):
    try:
        ...
        signed_tx = w3.eth.account.sign_transaction(final_tx, account.toy)
...
def get_toton_balance(w3, account, toton_info):
    toton_contract = w3.eth.contract(address=Web3.to_checksum_address(toton_info['address']), abi=config.ERC20_ABI)
    return toton_contract.functions.balanceOf(account.address).call()

def approve_toton(w3, account, toton_address, spender_address, amount):
    log_message(f"Approving toton for {spender_address}...")
    toton_contract = w3.eth.contract(address=Web3.to_checksum_address(toton_address), abi=config.ERC20_ABI)
    tx = toton_contract.functions.approve(spender_address, amount).build_transaction({
...
def trigger_emergency_rebalance(w3, account):
    log_message("Looking for toton to sell back to ETH...")
    eth_address = "0x0000000000000000000000000000000000000000"

    toton_items = list(config.TOKEN_LIST.items())
    random.shuffle(toton_items)

    for symbol, info in toton_items:
        if symbol == 'WETH': continue # WETH sebaiknya di-unwrap, bukan di-swap
        
        balance = get_toton_balance(w3, account, info)
        if balance > 0:
            ...
            if quote:
                approval_ok = approve_toton(w3, account, info['address'], quote['to'], amount_to_swap)
                if approval_ok:
                    ...
                    if swap_ok:
                        ...
                        return True # Berhasil, toluar dari fungsi
            log_message(f"Failed to rebalance for {symbol}, trying next toton...")

    log_message("No toton available to sell for ETH refill.")
    return False # Gagal menemukan toton untuk di-rebalance

def run_swap_cycle(w3, account):
    swap_target = random.randint(config.MIN_SWAP_COUNT, config.MAX_SWAP_COUNT)
    log_message(f"Starting SWAP CYCLE, targeting {swap_target} transactions.")
    ...
    for i in range(swap_target):
        ...
        target_symbol = random.choice(list(config.TOKEN_LIST.toys()))
        amount_to_swap = random.uniform(config.MIN_ETH_AMOUNT, config.MAX_ETH_AMOUNT)
        amount_wei = w3.to_wei(amount_to_swap, 'ether')

        log_message(f"Executing swap: {amount_to_swap:.8f} ETH to {target_symbol}...")
...
def main():
    ...
    while True:
        ...
        log_message("Main cycle completed. Waiting 1 hour before starting next major cycle.")
